...
public class register_controller implements Initializable {
    ...
    @FXML
    private TextField text_office,text_doc,text_regtype,text_regname,text_fee,
    ...
    private TableColumn<?, ?>col_regnum,col_regname,col_docnum,col_docname,
            col_regcount,col_regfee,col_unreg,col_regtime;
    @FXML
    ...
    public void initialize(URL location, ResourceBundle resources)
    {
        ...
        col_regfee.setCellValueFactory(new PropertyValueFactory<>("regCost"));
        ...
        try
        {
            ...
            String sql = "select depid,name from department";
            ResultSet rs = stmt.executeQuery(sql);
            String office_name,depid;
            while(rs.next())
            {
                depid = rs.getString("depid");
                office_name = rs.getString("name");
                ob_office.add(office_name);
                office_list.add(depid);
            }
            rs.close();
            stmt.close();
                                                                       ...
                                                                       String oldValue, String newValue)
                                                   {
                                                       ...
                                                       else if(pattern.matcher(newValue).matches())//非汉字
                                                       {
                                                           try
                                                           {
                                                               stmt = MainApp.conn.createStatement();
                                                               String sql = "select depid,name from department "
                                                                       + "where department.py like '"+newValue+"%'";
                                                               ResultSet rs = stmt.executeQuery(sql);
                                                               cut_ob_office.clear();
                                                               cut_of_list.clear();
                                                               String office_name,depid;
    ...
    {
        boolean flag_unreg=false,speciallist=false;
        String regnum,regname,docnum,docname,regcount,regfee,unreg,regtime;
        try
        {
            stmt = MainApp.conn.createStatement();
            String sql;
            ...
            if(rs.next())
            {
                ...
                {
                    ob_patreg.clear();
                    sql = "select reg1.reg_id,reg2.name,reg2.speciallist,reg1.docid,"
                            + "doc.name,reg1.current_reg_count,reg1.unreg,reg1.reg_fee,"
                            + "reg1.reg_datetime from register reg1,doctor doc,"
                            + "register_category reg2 where reg1.pid='"+MainApp.pat_doc_num+"' "
                            + "and doc.docid=reg1.docid and reg2.catid=reg1.catid";
                    rs = stmt.executeQuery(sql);
                    while(rs.next())
                    {
                        regnum = rs.getString("reg1.reg_id");
                        regname = rs.getString("reg2.name");
                        speciallist = rs.getBoolean("reg2.speciallist");
                        regname = regname+"（"+(speciallist?"专家":"普通")+"号）";
                        docnum = rs.getString("reg1.docid");
                        docname = rs.getString("doc.name");
                        regcount = rs.getString("reg1.current_reg_count");
                        regfee = rs.getString("reg1.reg_fee");
    ...
    private void on_btn_unreg_clicked(ActionEvent event)
    {
        ...
        for(int i=0;i<ob_unreg.size();++i)
        {
            ...
            {
                stmt = MainApp.conn.createStatement();
                String sql = "update register set unreg=1 where reg_id"
                        + "= '"+patreg.getRegNum()+"'";
                stmt.executeUpdate(sql);

                sql = "update register set current_reg_count=current_reg_count-1 where " +
                        "catid in ( select reg3.catid from " +
                        "(select catid from register " +
                        "where reg_id='"+patreg.getRegNum()+"') reg3)";
                stmt.executeUpdate(sql);
                stmt.close();
            }catch(SQLException se){
                // 处理 JDBC 错误
    ...
    private void on_text_doc_clicked(Event event)
    {
        ...
        else
        {
            ...
            try
            {
                doc_list.clear();
                stmt = MainApp.conn.createStatement();
                String sql = "select docid,name from doctor where depid ="
                        + "'"+sel_off_num+"'";
                ResultSet rs = stmt.executeQuery(sql);
                String name,docid;
                while(rs.next())
    ...
    private void on_text_regtype_clicked(Event event)
    {
        ...
        else
        {
            ...
            try {
                stmt = MainApp.conn.createStatement();
                String sql = "select speciallist from doctor where "
                        + "docid = '" + sel_doc_num + "'";
                ResultSet rs = stmt.executeQuery(sql);
                if (rs.next()) {
                    boolean speciallist = rs.getBoolean("speciallist");
                    if (speciallist) {
    ...
    private void on_text_regname_clicked(Event event)
    {
        ...
        text_regnum.clear();
        String office = text_office.getText();
        String specialliststr = text_regtype.getText();
        if(office==null||office.isEmpty()
                ||specialliststr==null||specialliststr.isEmpty())
        {
            context_regname.hide();
            JOptionPane.showMessageDialog(new JFrame().getContentPane(),
        ...
        int speciallist = specialliststr.equals("专家号") ? 1 :0;
        ob_regname.clear();
        reg_list.clear();
        try
        {
            stmt = MainApp.conn.createStatement();
            String sql = "select catid,name from register_category where "
                    + "depid = '"+sel_off_num+"' and "
    ...
    @FXML
    private void on_text_fee_clicked(Event event)
    {
        text_pay.clear();
        text_charge.clear();
        text_regnum.clear();
        ...
        else
        {
            try
            {
                double reg_fee;
                stmt = MainApp.conn.createStatement();
                String sql = "select reg_fee from register_category where "
                        + "catid = '"+sel_reg_num+"'";
                ResultSet rs = stmt.executeQuery(sql);
                if(rs.next())
                {
                    reg_fee = rs.getDouble("reg_fee");
                    text_fee.setText(Double.toString(reg_fee));
                }
                else
                {
                    JOptionPane.showMessageDialog(new JFrame().getContentPane(),
                ...
                sql = "select balance from patient where "
                        + "pid = '"+MainApp.pat_doc_num+"'";
                rs = stmt.executeQuery(sql);
                if(rs.next())
                {
                    double prestore = rs.getDouble("balance");
                    if(prestore>=reg_fee)
                    {
                        text_pay.setText(Double.toString(prestore));
                        text_pay.setEditable(false);
                        text_charge.setText(Double.toString(prestore-reg_fee));
                    }
                    else
                    {
                        text_pay.setPromptText("请缴纳至少"+reg_fee+"元");
    ...
    private void on_text_charge_clicked(Event event)
    {
        ...
        else
        {
            ...
            if(pay>=fee)
            {
                text_charge.setText(Double.toString(pay-fee));
            }
            else
            {
                JOptionPane.showMessageDialog(new JFrame().getContentPane(),
    ...
    private void on_btn_ok_clicked(ActionEvent event)
    {
        ...
        String regfee = text_fee.getText();
        int registernum=0;
        ...
        try
        {
            ...
            String sql = "select max_reg_number from register_category where "
                    + "catid='"+sel_reg_num+"'";
            ResultSet rs = stmt.executeQuery(sql);
            if(rs.next())
            {
                max_pat = rs.getInt("max_reg_number");
            }
            ...
            else
            {
                ...
                sql = "insert into register (reg_id,catid,"
                        + "docid,pid,current_reg_count,unreg,reg_datetime,reg_fee) "
                ...
                {
                    sql = "update patient set balance = "
                            + ""+charge+"+balance where "
                            + "pid= '"+MainApp.pat_doc_num+"'";
                }
                else
                {
                    sql = "update patient set balance = "
                            + ""+charge+"where pid= '"+MainApp.pat_doc_num+"'";
                }
                stmt.executeUpdate(sql);

                sql = "update register set current_reg_count= '"+(1+regcount)+"' where "
                        + "catid='"+sel_reg_num+"' and reg_datetime>="
                        + "'"+currtime+"'";
                stmt.executeUpdate(sql);
